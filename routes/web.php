...
if (env('APP_ENV') == 'production') // Route untuk server Production
{
    ...
    // V-CLaim Routes
    $router->group(['prefix' => 'vclaim-rest'], function () use ($router) {
        // LPK Routes
...
else // Routes for Development Server
{
    $router->group(['prefix' => 'bpjs-rest-dev'], function () use ($router) {
        $router->get('{serviceName}/{url}', 'GeneralController@apiData');
        $router->get('{serviceName}/{url}/{param}', 'GeneralController@getByParam');
    });
    
    // V-CLaim Routes
    $router->group(['prefix' => 'vclaim-rest-dev'], function () use ($router) {
        // LPK Routes
        $router->group(['prefix' => 'LPK'], function () use ($router) {
            $router->post('insert', 'VClaimController@apiData');
            $router->put('update', 'VClaimController@apiData');
    ...
    // Apotek Routes
    $router->group(['prefix' => 'apotek-rest-dev'], function () use ($router) {
        // Referensi Routes
        $router->group(['prefix' => 'referensi'], function () use ($router) {
            $router->get('dpho', 'ApotekController@apiData');
            $router->get('poli/{param}', 'ApotekController@apiData');
